...
class ProfileController extends Controller
{
    public function show(Request $request, $doctorId = null)
    {
        ...
        // If a doctor is authenticated via session, show the doctor profile
        if ($request->has('current_user') && isset($request->current_user['type']) && $request->current_user['type'] === 'doctor') {
            $doctor = \App\Models\Doctor::with(['school', 'healthFacility', 'appointments' => function($q) {
                $q->latest()->take(5);
            }, 'availabilities'])->find($request->current_user['id']);
